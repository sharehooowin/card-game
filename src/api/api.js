...
class Api{
    ...
    * */
     ...
     * data:接口参数
     */
    async getResult(name='login',data='',headers={}){
        ...
        if(apiInfo.method==='GET'){
            ...
            result = await this.httpGet(url,Object.assign({},apiInfo.header,headers))
        }else if(apiInfo.method==='POST'){
            if(apiInfo.disableBody){
                let url = apiInfo.url;
                url += "?"
                for(let i in data){
                    url += i + "=" +  data[i] + "&";
                }
                result = await this.httpPost(url,null,Object.assign({},apiInfo.header,headers))
            }else{
                result = await this.httpPost(apiInfo.url,data,Object.assign({},apiInfo.header,headers))
            }
        }
        return result
    }
